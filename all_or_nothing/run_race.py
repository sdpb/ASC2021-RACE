...
def PRINT_DEBUG(var, ex=True):
    print(f"DEBUG::: -- TYPE: {type(var)} -- DATA: {var}", end="")
    if type(var) == type([]):
...
def main():
    ...
    accelerator = Accelerator(fp16=args.fp16, device_placement=True)
    n_gpu=1
    ...
    if is_main_process():
        logger.info("device: {} n_gpu: {}, distributed training: {}, 16-bits training: {}".format(
            accelerator.device, n_gpu, bool(args.local_rank != -1), args.fp16))
    ...
    if is_main_process():
        ...
        accelerator.save(unwrapped_model.state_dict(), output_model_file)
