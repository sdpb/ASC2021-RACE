...
def training_function(config, args):
    ...
    PRINT_DEBUG(tokenized_datasets["train"].features)
    # Instantiate dataloaders.
    #PRINT_DEBUG(tokenized_datasets["train"])
    ...
    for epoch in range(num_epochs):
        model.train()
        #print(f"TYPE:{type(train_dataloader)} -- TRAIN_DATALOADER:{train_dataloader}")
        for step, batch in enumerate(train_dataloader):
            print(f"TYPE:{type(batch)} -- BATCH:{batch}" )
            # We could avoid this line since we set the accelerator with `device_placement=True`.
            #PRINT_DEBUG(batch.data)
